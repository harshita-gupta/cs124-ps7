...
We can count the edges in the cut in the following way: consider any vertex v $\in$ S1. For every vertex w in $S_2...S_k$ that it is connected to by an edge, we add 1/2 to a running sum. We do the same for each vertex in $S_2...S_k$. Note that each edge crossing the cut contributes 1 to the sum– 1/2 for each vertex of the edge.
...
Due to our usage of local search, at least $\frac{1}{k}$ of the edges from any vertex $v$ must lie in sets other than $v$, otherwise, we could move the set of $v$, and improve the cut. This implies that:
\begin{multline*}
	C \geq 
\end{multline*}
